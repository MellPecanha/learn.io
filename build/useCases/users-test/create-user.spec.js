...
// src/useCases/users-test/create-user.spec.ts
describe("Create User Use Case", () => {
  ...
  it("deve ser poss\xEDvel cadastrar um novo usu\xE1rio", async () => {
    ...
    globalExpect(user?.id).toEqual(globalExpect.any(Number));
    globalExpect(user?.username).toBe("novo_usuario");
    globalExpect(usersRepository.items).toHaveLength(1);
  });
  it("deve cadastrar o usu\xE1rio com o role correto", async () => {
    const user = await sut.execute({
    ...
    globalExpect(user?.role).toBe("PROFESSOR" /* PROFESSOR */);
  });
});
