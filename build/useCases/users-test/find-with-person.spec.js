...
// src/useCases/users-test/find-with-person.spec.ts
describe("Find With Person Use Case", () => {
  ...
  it("deve ser poss\xEDvel buscar um usu\xE1rio com seus dados de pessoa", async () => {
    const result = await sut.execute(1);
    globalExpect(result).toHaveProperty("cpf");
    globalExpect(result?.name).toBe("Fake User Name");
  });
  it("deve lan\xE7ar erro ao buscar um usu\xE1rio inexistente", async () => {
    await globalExpect(() => sut.execute(999)).rejects.toBeInstanceOf(
