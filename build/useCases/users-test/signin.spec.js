...
// src/useCases/users-test/signin.spec.ts
describe("Signin Use Case", () => {
  ...
  beforeEach(async () => {
    ...
    await usersRepository.create({
      ...
      // No seu Use Case atual, a senha não é validada
      role: "PROFESSOR" /* PROFESSOR */
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    });
  });
  it("deve ser poss\xEDvel autenticar com um username v\xE1lido", async () => {
    const user = await sut.execute("professor_teste");
    globalExpect(user.id).toEqual(globalExpect.any(Number));
    globalExpect(user.username).toBe("professor_teste");
  });
  it("n\xE3o deve ser poss\xEDvel autenticar com um username inexistente", async () => {
    await globalExpect(
