...
// src/useCases/posts-test/search-posts.spec.ts
describe("Search Posts Use Case", () => {
  ...
  beforeEach(async () => {
    ...
    await postsRepository.create({
      title: "JavaScript Avan\xE7ado",
      content: "Conte\xFAdo JS"
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    });
  });
  it("deve ser poss\xEDvel pesquisar posts como ALUNO", async () => {
    const posts = await sut.execute("JavaScript", "ALUNO" /* ALUNO */);
    globalExpect(posts).toHaveLength(1);
    globalExpect(posts[0].title).toBe("JavaScript Avan\xE7ado");
  });
  it("deve retornar uma lista vazia se o termo de busca n\xE3o for encontrado", async () => {
    const posts = await sut.execute("termo_inexistente", "ALUNO" /* ALUNO */);
    globalExpect(posts).toHaveLength(0);
    globalExpect(posts).toEqual([]);
  });
});
