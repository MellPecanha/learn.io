...
// src/useCases/posts-test/create-posts.spec.ts
describe("Create Posts Use Case", () => {
  ...
  it("deve ser poss\xEDvel criar um post como PROFESSOR", async () => {
    const postData = {
      title: "T\xEDtulo de Teste",
      content: "Conte\xFAdo do post de teste",
      image_url: "http://imagem.com/teste.jpg",
      author_id: 1
    };
    const post = await sut.execute(postData, "PROFESSOR" /* PROFESSOR */);
    globalExpect(post.id).toEqual(globalExpect.any(Number));
    globalExpect(post.title).toBe("T\xEDtulo de Teste");
    globalExpect(postsRepository.items).toHaveLength(1);
  });
  it("n\xE3o deve ser poss\xEDvel criar um post como ALUNO", async () => {
    const postData = {
      title: "T\xEDtulo de Teste",
      content: "Conte\xFAdo"
    };
    await globalExpect(
      () => (
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sut.execute(postData, "ALUNO" /* ALUNO */)
      )
    ).rejects.toThrow();
  });
});
