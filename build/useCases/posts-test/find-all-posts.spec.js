...
// src/useCases/posts-test/find-all-posts.spec.ts
  ...
  beforeEach(async () => {
    ...
    for (let i2 = 1; i2 <= 3; i2++) {
      await postsRepository.create({
        title: `Post ${i2}`,
        content: `Conte\xFAdo ${i2}`
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      });
    }
  });
  it("deve ser poss\xEDvel listar todos os posts como ALUNO", async () => {
    const posts = await sut.execute(1, 2, "ALUNO" /* ALUNO */);
    globalExpect(posts).toHaveLength(2);
  });
  it("deve ser poss\xEDvel listar todos os posts como PROFESSOR", async () => {
    const posts = await sut.execute(1, 10, "PROFESSOR" /* PROFESSOR */);
    globalExpect(posts).toHaveLength(3);
  });
  it("n\xE3o deve ser poss\xEDvel listar posts sem um role v\xE1lido", async () => {
    await globalExpect(() => sut.execute(1, 10, "ROLE_INVALIDA")).rejects.toThrow();
  });
});
