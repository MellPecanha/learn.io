...
// src/useCases/posts-test/update-post.spec.ts
describe("Update Post Use Case", () => {
  ...
  it("deve ser poss\xEDvel atualizar um post existente como PROFESSOR", async () => {
    const updated = await sut.execute(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      { id: 1, title: "Novo T\xEDtulo" },
      "PROFESSOR" /* PROFESSOR */
    );
    globalExpect(updated.title).toBe("Novo T\xEDtulo");
    globalExpect(postsRepository.items[0].title).toBe("Novo T\xEDtulo");
  });
  it("n\xE3o deve ser poss\xEDvel atualizar um post como ALUNO", async () => {
    await globalExpect(
      () => (
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sut.execute({ id: 1, title: "Novo" }, "ALUNO" /* ALUNO */)
      )
    ).rejects.toThrow();
  });
});
