...
var FixtureDependencyError = class extends Error {
  constructor() {
    super(...arguments);
    __publicField(this, "name", "FixtureDependencyError");
  }
};
var FixtureAccessError = class extends Error {
  constructor() {
    super(...arguments);
    __publicField(this, "name", "FixtureAccessError");
  }
};
var FixtureParseError = class extends Error {
  constructor() {
    super(...arguments);
    __publicField(this, "name", "FixtureParseError");
  }
};
...
var _TestFixtures = class _TestFixtures {
  constructor(registrations) {
    __publicField(this, "_suiteContexts");
    __publicField(this, "_overrides", /* @__PURE__ */ new WeakMap());
  ...
  parseUserFixtures(runner2, userFixtures, supportNonTest, registrations = new Map(this._registrations)) {
    ...
    Object.entries(userFixtures).forEach(([name, fn2]) => {
      ...
      let value;
      let _options;
      if (Array.isArray(fn2) && fn2.length >= 2 && isObject(fn2[1]) && Object.keys(fn2[1]).some((key) => _TestFixtures._fixtureOptionKeys.includes(key))) {
        _options = fn2[1];
        options = {
...
__publicField(_TestFixtures, "_definitions", []);
__publicField(_TestFixtures, "_builtinFixtures", [
  "task",
  "signal",
  "onTestFailed",
  "onTestFinished",
...
__publicField(_TestFixtures, "_fixtureOptionKeys", [
  "auto",
  "injected",
  "scope"
]);
__publicField(_TestFixtures, "_fixtureScopes", [
  "test",
  "file",
  "worker"
]);
__publicField(_TestFixtures, "_workerContextSuite", { type: "worker" });
var TestFixtures = _TestFixtures;
...
// node_modules/vitest/dist/chunks/test.CTcmp4Su.js
var unsupported = [
  "matchSnapshot",
  "toMatchSnapshot",
...
// src/useCases/address-test/find-address-by-person.spec.ts
describe("Find Address By Person Use Case", () => {
  ...
  it("deve ser poss\xEDvel listar endere\xE7os de uma pessoa espec\xEDfica", async () => {
    const addresses = await sut.execute(1, 1, 10);
    globalExpect(addresses).toHaveLength(2);
    globalExpect(addresses[0].street).toBe("Rua A");
    globalExpect(addresses[1].street).toBe("Rua B");
  });
  it("deve retornar uma lista vazia se a pessoa n\xE3o tiver endere\xE7os", async () => {
    const addresses = await sut.execute(999, 1, 10);
    globalExpect(addresses).toHaveLength(0);
  });
  it("deve ser poss\xEDvel paginar a busca de endere\xE7os", async () => {
    const addresses = await sut.execute(1, 2, 1);
    globalExpect(addresses).toHaveLength(1);
    globalExpect(addresses[0].street).toBe("Rua B");
  });
});
