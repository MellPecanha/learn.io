...
// src/useCases/address-test/create-address.spec.ts
describe("Create Address Use Case", () => {
  ...
  it("deve ser poss\xEDvel cadastrar um novo endere\xE7o", async () => {
    ...
    globalExpect(address?.id).toEqual(globalExpect.any(Number));
    globalExpect(address?.street).toBe("Rua de Teste");
    globalExpect(addressRepository.items).toHaveLength(1);
  });
  it("n\xE3o deve ser poss\xEDvel criar endere\xE7o sem uma pessoa vinculada", async () => {
    await globalExpect(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      () => sut.execute({ street: "Rua" })
      // Sem person_id
    ).rejects.toThrow();
  });
});
